...
class _HandlerShim:
    ...
    def __init__(self, model: torch.nn.Module, device: str, dtype: torch.dtype) -> None:
        """
        Initialize the handler shim with the provided model, device, and dtype.
        
        Parameters:
            model (torch.nn.Module): The model instance exposed to the original trainer.
            device (str): Device identifier to associate with the handler (e.g., "cpu" or "cuda").
            dtype (torch.dtype): Default tensor dtype to use for the handler's model.
        
        Notes:
            This sets the attributes `model`, `device`, and `dtype` on the instance and initializes
...
def run_vanilla(args: argparse.Namespace) -> int:
    """
    Run the vanilla training subcommand using the original LoRA trainer API.
    
    Performs UI setup, GPU/precision detection, loads the model, constructs LoRA and training configurations, invokes the original trainer, tracks progress, and shows a summary.
    
    Returns:
        int: 0 on success, 1 on general failure, 130 if interrupted by the user (Ctrl+C).
    """
    # -- UI setup -------------------------------------------------------------
