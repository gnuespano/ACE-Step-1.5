...
def build_configs(args: argparse.Namespace) -> Tuple[LoRAConfigV2, TrainingConfigV2]:
    """
    Construct LoRAConfigV2 and TrainingConfigV2 from parsed CLI arguments.
    
    If a model config.json exists under the checkpoint variant directory, patches
    `timestep_mu`, `timestep_sigma`, and `data_proportion` from that file into the
    returned training configuration; otherwise defaults are used.
    
    Parameters:
        args (argparse.Namespace): Parsed CLI arguments produced by argparse.
    
    Returns:
        tuple: (lora_cfg, train_cfg) where `lora_cfg` is a LoRAConfigV2 built from
        relevant CLI flags and `train_cfg` is a TrainingConfigV2 populated from CLI
        flags, GPU detection, and the optional model config.json values.
    """
