...
def run_fixed(args: argparse.Namespace) -> int:
    """
    Run the "fixed" training subcommand: build configs, load the model, and execute training using FixedLoRATrainer.
    
    This function constructs LoRA and training configurations from CLI arguments, optionally displays a banner and configuration for confirmation, loads the decoder model for training, runs the training loop while tracking progress, and shows a final summary. It handles user interruption (Ctrl+C) and reports model-loading or training errors via the UI error handler.
    
    Parameters:
        args (argparse.Namespace): Parsed CLI arguments used to build configs and control UI/confirmation.
    
    Returns:
        int: Process exit code — `0` on success, `130` if interrupted by the user (Ctrl+C), or `1` on error.
    """
    import torch
    ...
    # -- Banner & config (skip if wizard already showed them) -----------------
    if not getattr(args, "_from_wizard", False):
        ...
        show_config(lora_cfg, train_cfg, subcommand="fixed")
