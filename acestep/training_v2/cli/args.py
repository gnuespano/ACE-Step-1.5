...
def _add_model_args(parser: argparse.ArgumentParser) -> None:
    """
    Add model path and variant arguments to the given ArgumentParser.
    
    Adds a "Model / paths" argument group containing:
    - --checkpoint-dir: required path to the checkpoints root directory.
    - --model-variant: model variant choice ("turbo", "base", "sft"), default "turbo".
    
    Parameters:
        parser (argparse.ArgumentParser): The parser to extend with these arguments.
    """
...
def _add_device_args(parser: argparse.ArgumentParser) -> None:
    """
    Register device and precision command-line arguments on the provided parser.
    
    Adds a "Device / platform" argument group containing:
    - `--device`: Device specifier (default "auto"). Accepts values like "auto", "cuda", "cuda:0", "mps", "xpu", or "cpu" to select the target execution device.
    - `--precision`: Numeric precision mode (default "auto"). Must be one of "auto", "bf16", "fp16", or "fp32" to control model/data precision.
    """
...
def _add_fixed_args(parser: argparse.ArgumentParser) -> None:
    """
    Add a "Corrected training" argument group and register the --cfg-ratio option.
    
    The --cfg-ratio option controls the classifier-free guidance (CFG) dropout probability and defaults to 0.15.
    """
...
def _add_selective_args(parser: argparse.ArgumentParser) -> None:
    """
    Add the "Selective / estimation" argument group used by the selective subcommand.
    
    Adds the --module-config option (path to a JSON module config), the --auto-estimate flag (run estimation inline before training), and the estimation-related arguments required for selective training.
    """
