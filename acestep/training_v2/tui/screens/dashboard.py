...
class DashboardScreen(Screen):
    ...
    def __init__(self) -> None:
        """
        Initialize the DashboardScreen and select a random display motto.
        
        Chooses one entry from the module-level MOTTOS list at random and stores it on the instance as `self._motto`.
        """
        super().__init__()
        import random
    ...
    def compose(self) -> ComposeResult:
        """
        Builds and yields the widget tree for the dashboard screen layout.
        
        Constructs the dashboard UI by yielding a banner (ASCII art, version tag, and rotating motto), a row of quick-action buttons, the main content area (recent runs table, quick stats including GPU status, and a workflow guidance panel), and a navigation footer (datasets, history, settings, quit).
        
        Returns:
            ComposeResult: An iterable of widgets and containers representing the composed dashboard UI.
        """
        # Banner
    ...
    def _start_gpu_monitor(self) -> None:
        """
        Start GPU status monitoring by performing an immediate update and scheduling recurring updates every 2 seconds.
        """
    ...
    def _update_gpu_status(self) -> None:
        """
        Refresh dashboard GPU metrics and propagate them to application state.
        
        Fetches current GPU information, updates the VRAM and utilization widgets, and writes the GPU status (vram_used_gb, vram_total_gb, utilization, name) into the application's state. If fetching or updating fails, sets the VRAM and utilization displays to "N/A".
        """
        try:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        """
        Dispatches dashboard actions based on the pressed button's ID.
        
        Calls the corresponding screen action method for dashboard buttons (fixed training, vanilla training, preprocess/browse, estimate, history, settings) or exits the application when the quit button is pressed.
        
        Parameters:
            event (Button.Pressed): The button press event whose button.id is used to determine the action.
        """
    ...
    def action_fixed_training(self) -> None:
        """
        Initiates the workflow to create a new fixed training run.
        
        Starts the UI flow for configuring and launching a fixed-training job.
        """
    ...
    def action_preprocess(self) -> None:
        """
        Navigate the application to the preprocessing (dataset browser) screen.
        """
        self.app.action_goto_preprocess()
    
    def action_estimate(self) -> None:
        """Navigate to the gradient estimation screen."""
    ...
    def action_refresh(self) -> None:
        """
        Refresh the dashboard UI and GPU status, then show a brief notification.
        
        Updates displayed runs and statistics, refreshes GPU metrics, and notifies the user with "Dashboard refreshed".
        """
        self._refresh_data()
        self._update_gpu_status()
