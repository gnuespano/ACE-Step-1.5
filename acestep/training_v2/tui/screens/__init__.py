...
def __getattr__(name: str):
    """
    Lazily imports and returns a screen class from the package's screens submodules when the attribute is accessed.
    
    Parameters:
        name (str): The attribute name of the requested screen class (one of
            "DashboardScreen", "TrainingConfigScreen", "TrainingMonitorScreen",
            "DatasetBrowserScreen", "PreprocessMonitorScreen", "RunHistoryScreen",
            "SettingsScreen", "EstimateConfigScreen", "EstimateMonitorScreen").
    
    Returns:
        type: The requested screen class object.
    
    Raises:
        AttributeError: If `name` is not a recognized exported screen identifier.
    """
