...
class TrainingMonitorScreen(Screen):
    ...
    def __init__(
        ...
        """
        Initialize the TrainingMonitorScreen with run metadata and runtime configuration.
        
        Parameters:
            run (RunInfo): Metadata for the training run being monitored.
            config (Dict[str, Any]): Runtime configuration for the training session.
    ...
    def compose(self) -> ComposeResult:
        """
        Builds and yields the UI composition for the training monitor screen.
        
        The composition includes header controls (Pause, Stop, Back), an epoch progress section with ETA,
        main metrics panels (loss sparkline and GPU gauge), current/best statistics, and a scrolling training log.
        
        Returns:
            ComposeResult: an iterable of widgets that compose the training monitor layout.
        """
    ...
    def on_mount(self) -> None:
        """
        Initialize monitoring state and start the background training run when the screen is mounted.
        
        Sets the run start time, notifies the application state to begin the run, writes initial header and run metadata to the training log viewer, starts the background training worker, and schedules periodic draining of the internal log queue for UI updates.
        """
    ...
    def _run_training(self) -> None:  # NOTE: sync, not async
        """
        Prepare LoRA and training configurations from the screen's config, choose the appropriate trainer implementation, and run training on the current background worker; on error, enqueue an error message and signal training completion as failed.
         
        This method constructs a LoRA config and a TrainingConfigV2 using values from self.config, then dispatches execution to either _run_vanilla or _run_fixed depending on the run's trainer_type. If an exception occurs, an error message is pushed to the internal log queue and _on_training_complete is invoked with False.
        """
    ...
    def _run_fixed(
        ...
        """
        Run a FixedLoRATrainer using the provided LoRA and training configurations, forwarding progress and log messages to the UI.
        
        Parameters:
            lora_config: Configuration object that specifies LoRA parameters and target modules for the trainer.
            training_config: Training configuration object (paths, device/precision, optimizer, epochs, etc.). The resolved device and precision may be updated on this object.
            worker: Background worker object used to detect cancellation (must expose an `is_cancelled` attribute).
        """
    ...
    def _run_vanilla(
        ...
        """
        Run the VanillaTrainer loop using the provided LoRA and training configurations and forward progress to the UI.
        
        Parameters:
            lora_config: Configuration for LoRA adaptation passed to the trainer.
            training_config: Training configuration (hyperparameters, device, paths, etc.) passed to the trainer.
            worker: Background worker or cancellation token used by the training loop; checked indirectly by the progress callback.
        
        Detailed behavior:
            - Enqueues log messages when loading and starting training.
            - Creates a VanillaTrainer with a thread-safe progress callback that updates UI progress, logs epoch completions, and respects the screen's pause/stop flags.
            - Starts trainer.train(), then enqueues a success or stopped message and calls _on_training_complete(True) on success or _on_training_complete(False) if stopped by the user.
        """
        ...
        def _callback(
            ...
            **kwargs,
            ...
            """
            Progress callback invoked by the trainer to report step-level progress, update the UI, and control pause/stop behavior.
            ...
            Parameters:
                ...
                is_epoch_end (bool): True when the callback is called at the end of an epoch.
                **kwargs: Additional unused keyword arguments forwarded by trainer.
            
            Returns:
                bool: `True` if training should continue, `False` to request stopping.
            """
            self.app.call_from_thread(
    ...
    def _update_progress(
        ...
        """
        Update visible training metrics and persist the latest progress state.
        
        Parameters:
            epoch (int): Current epoch index (1-based).
            step (int): Current step/iteration within the epoch.
            loss (float): Latest loss value for the current step.
            lr (float): Current learning rate; ignored if not positive.
            is_epoch_end (bool): True when this update corresponds to the end of an epoch; triggers an epoch-end log entry.
        """
        try:
    ...
    def _on_training_complete(self, success: bool) -> None:
        """
        Handle end-of-training cleanup and user notification.
        
        Updates the application run state with the final success flag, disables the Pause and Stop controls, and displays a short notification indicating whether the run completed successfully or was stopped/failed.
        
        Parameters:
            success (bool): True if training finished successfully, False if it was stopped or failed.
        """
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        """
        Route UI button-press events to their corresponding action handlers.
        
        Handles the following button IDs:
        - "btn-pause": toggles pause/resume via action_toggle_pause.
        - "btn-stop": requests stop via action_stop_training.
        - "btn-back": navigates back via action_back.
    ...
    def action_toggle_pause(self) -> None:
        """
        Toggle the training pause state and update UI, logs, and application state accordingly.
        
        Flips the internal paused flag, updates the Pause/Resume button label, emits a log message to the training log viewer, shows a short user notification, and calls the application state's pause_run() or resume_run() method.
        """
    ...
    def action_stop_training(self) -> None:
        """
        Signal the running training worker to stop and update the UI and internal state.
        
        Sets the internal stopping flag, clears any paused state so the worker can exit,
        writes a warning to the training log, shows a short user notification, and cancels
        the background training worker if present.
        """
    ...
    def action_back(self) -> None:
        """
        Navigate back to the previous screen, preventing exit while a training run is active.
        
        If a training run is currently running and not already stopping, notifies the user to stop the run and aborts navigation.
        """
