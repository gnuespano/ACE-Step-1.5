...
class EstimateConfigScreen(Screen):
    ...
    def __init__(self, name: Optional[str] = None) -> None:
        """
        Initialize the screen with an optional name.
        
        Parameters:
            name (Optional[str]): Optional screen identifier used by the UI framework. Defaults to None.
        """
        super().__init__(name=name)
    
    def compose(self) -> ComposeResult:
        """
        Builds and yields the UI layout for the gradient sensitivity estimation configuration screen.
        
        Creates the header, information panel, required settings (checkpoint, model variant, dataset, output),
        and estimation parameter controls (batches, top-k, granularity, batch size) as the ComposeResult for this screen.
        
        Returns:
            ComposeResult: An iterable of UI widgets that make up the estimation config form.
        """
        # Header
    ...
    def _get_config(self) -> Dict[str, Any]:
        """
        Collect form values and produce a configuration dictionary for the estimation run.
        
        Returns:
            dict: Configuration with keys:
                - `checkpoint_dir` (str): Filesystem path to the model checkpoint directory.
                - `dataset_dir` (str): Filesystem path to the dataset directory.
                - `output_file` (str): Path to the output JSON file to write results.
                - `variant` (str): Selected model variant (e.g., "turbo", "base", "sft").
                - `num_batches` (int): Number of batches to process; defaults to 10 when not set.
                - `top_k` (int): Number of top modules to keep; defaults to 16 when not set.
                - `granularity` (str): Selected granularity value (e.g., "module" or "layer").
                - `batch_size` (int): Batch size for processing; defaults to 1 when not set.
        """
        return {
    ...
    def _get_selected_variant(self) -> str:
        """
        Determine which model variant radio button is selected.
        
        Returns:
            str: One of "turbo", "base", or "sft". Returns "turbo" if no variant is selected or an error occurs.
        """
        try:
    ...
    def _validate(self) -> Optional[str]:
        """
        Validate the configuration form and return the first validation error found.
        
        Checks that checkpoint directory, dataset directory, and output file path are provided
        and that the checkpoint directory exists.
        
        Returns:
            str: Error message describing the first failing validation.
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        """
        Dispatches button press events to the corresponding screen actions or file-picker dialogs.
        
        Parameters:
            event (Button.Pressed): The button press event whose button id determines which action to invoke
            - "btn-start": start estimation
            - "btn-back": go back
            - "btn-browse-checkpoint": open checkpoint directory picker
            - "btn-browse-dataset": open dataset directory picker
            - "btn-browse-output": open output path picker
        """
    ...
    def _open_file_picker(self, title: str, target_input_id: str) -> None:
        """
        Open a file-picker modal and update the specified Input widget with the selected path.
        
        Parameters:
            title (str): Title shown on the file picker modal.
            target_input_id (str): Query selector or widget ID of the Input to populate with the chosen path.
        """
        ...
        async def _handle_result(path) -> None:
            """
            Update the target input widget with the given path when a selection is provided.
            
            Parameters:
                path (str | os.PathLike | None): The selected filesystem path; if `None`, no update is performed.
            
            Notes:
                Any errors encountered while updating the input widget are suppressed.
            """
            if path is not None:
                try:
    ...
    def action_start(self) -> None:
        """
        Begin the estimation flow and navigate to the monitor screen.
        
        Validates the current form; if validation fails, shows an error notification and aborts.
        On success, builds the estimation configuration from the form and pushes an EstimateMonitorScreen
        initialized with that configuration.
        """
    ...
    def action_back(self) -> None:
        ...
        Pops the current screen from the application's screen stack.
        """
        self.app.pop_screen()
...
class EstimateMonitorScreen(Screen):
    ...
    def __init__(self, config: Dict[str, Any], name: Optional[str] = None) -> None:
        """
        Create an EstimateMonitorScreen configured for a specific estimation run and initialize its runtime state.
        
        Initializes the screen with the provided estimation `config` and sets up internal state used during background execution: cancellation flag, optional worker handle, a thread-safe log queue, and an empty results list.
        
        Parameters:
            config (Dict[str, Any]): Estimation configuration produced by the configuration screen (paths and estimation parameters).
            name (Optional[str]): Optional screen name used by the UI framework.
        """
        super().__init__(name=name)
        self.config = config
    ...
    def on_mount(self) -> None:
        """
        Initialize the results view and start the background estimation when the screen is mounted.
        
        Initializes the results DataTable, writes startup information to the LogViewer (dataset, batches, top_k), launches the background worker that runs the estimation, and schedules periodic processing of the queued log messages.
        """
    ...
    def _run_estimation(self) -> None:  # sync, runs in a thread
        """
        Execute the gradient sensitivity estimation using the screen's configuration inside the worker thread.
        
        Runs the estimator with the configured parameters, writes successful results to the configured output JSON file, enqueues informational/error/success messages to the screen's log queue, and schedules _on_complete(success: bool) to run on the app main thread. If the estimator module is unavailable or any exception occurs, an error is logged and completion is scheduled with a failure flag. """
    ...
    def _progress_callback(self, batch: int, total: int, module_name: str = "") -> None:
        """
        Update the on-screen progress and enqueue an informational log for the current module; intended to be called from a background worker thread.
        
        Parameters:
            batch (int): Index of the current batch (1-based or as provided by the worker).
            total (int): Total number of batches; if zero, progress is reported as 0%.
            module_name (str): Optional name of the module being processed; when provided an info log is queued.
        """
    ...
    def _update_progress(self, batch: int, total: int, percent: float) -> None:
        """
        Update the on-screen progress bar and label to reflect the current batch and completion percentage.
        
        Parameters:
            batch (int): Index of the current batch (1-based).
            total (int): Total number of batches.
            percent (float): Completion percentage from 0 to 100.
        """
    ...
    def _process_log_queue(self) -> None:
        """
        Drain the internal log queue and forward each entry to the estimate LogViewer.
        
        Reads all queued (level, message) tuples from self._log_queue and sends them to the LogViewer widget with the corresponding method for recognized levels: `info`, `warning`, `error`, and `success`.
        """
        log = self.query_one("#estimate-log", LogViewer)
    ...
    def _on_complete(self, success: bool) -> None:
        """
        Finalize the UI state and notify the user when an estimation run finishes.
        
        When `success` is True and results are available, enables the "Use" button,
        populates the results table, and shows a success notification. Always disables
        the "Cancel" button and enables the "Back" button. If not successful, shows a
        warning notification.
        
        Parameters:
            success (bool): True if the estimation completed successfully, False otherwise.
        """
    ...
    def _populate_results(self) -> None:
        """
        Fill the results DataTable with the top 20 sensitivity results from self._results.
        
        Each row contains rank (1-based), module name, and sensitivity formatted to six decimal places. If a result lacks a 'module' key the 'name' key is used; missing sensitivity falls back to 0.
        """
        table = self.query_one("#results-table", DataTable)
        table.clear()
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        """
        Dispatches UI actions based on which header button was pressed.
        
        Maps button IDs to screen actions:
        - "btn-cancel" -> cancels the running estimation
        - "btn-back" -> navigates back / closes the screen
        - "btn-use" -> applies or imports the latest estimation results
        
        Parameters:
            event (Button.Pressed): The button press event; the handler inspects event.button.id to determine the action.
        """
    ...
    def action_use_results(self) -> None:
        """
        Mark the current estimation output as the last used result and navigate back to the dashboard.
        
        If no results are available, a warning notification is shown and no state changes occur. Otherwise, the function stores the output file path from the current config in the application's state as the last estimation, displays a notification with the suggested `--module-config` CLI argument, and navigates to the dashboard.
        """
