...
class PreprocessMonitorScreen(Screen):
    ...
    def __init__(self, config: Dict[str, Any], name: Optional[str] = None) -> None:
        """
        Initialize the screen and its internal state using the provided preprocessing configuration.
        
        Parameters:
            config (Dict[str, Any]): Preprocessing configuration containing keys used by the screen and worker (e.g. "source_dir", "output_dir", "checkpoint_dir", "variant", "max_duration").
            name (Optional[str]): Optional screen name for the Textual framework.
        """
    ...
    def compose(self) -> ComposeResult:
        """
        Builds and yields the UI layout for the preprocessing monitor screen.
        
        Creates the screen structure including:
        - a header with the preprocessing title and Cancel/Back controls,
        - a progress section with a status label, progress bar, and current-file display,
        - a stats section showing processed, failed, and total counts,
        - a log section containing the preprocessing log viewer.
        """
    ...
    def _run_preprocessing(self) -> None:  # sync, runs in a thread
        """
        Execute the configured preprocessing workflow and report progress and completion to the UI.
        
        Searches the configured source directory for audio files or JSON manifests, ensures the output directory exists, and runs preprocessing either via the internal preprocessing API or a CLI fallback. Progress and status messages are posted to the screen's log queue and progress/stats are propagated to the UI; cancellation requests are honored. On completion or failure this method schedules a call to _on_complete(True) for success or _on_complete(False) for cancellation/failure.
        """
    ...
    def _run_cli_preprocessing(self) -> None:
        """
        Run preprocessing using the project's CLI as a fallback when the Python API is unavailable.
        
        Executes the project's train.py with the "preprocess" command (resolved from the repository root), streams stdout/stderr into the screen's log queue, and attempts to extract and forward simple progress updates to the UI. If cancellation is requested the subprocess is terminated. On subprocess exit the method enqueues a success or error message and invokes the screen's completion handler with the final success state.
        """
        import subprocess
        import sys
    ...
    def _progress_callback(
        ...
        """
        Handle a single preprocessing progress update: update counters, enqueue a log message, and schedule a UI progress refresh.
        
        Parameters:
        	current (int): The current number of items processed.
        	total (int): The total number of items to process.
        	filename (str): The path or name of the file associated with this progress update.
        	success (bool): Whether processing of `filename` succeeded; if `False`, increments the failed counter and logs a warning, otherwise logs an info message.
        """
    ...
    def _update_progress(self, current: int, total: int, filename: str = "") -> None:
        """
        Refresh the on-screen progress indicators for preprocessing.
        
        @param current (int): Number of files processed so far.
        @param total (int): Total number of files to process; used to compute percentage.
        @param filename (str): Path to the currently processed file; when provided, its basename is shown.
        """
    ...
    def _update_total(self, total: int) -> None:
        """
        Set the total number of items to process and refresh the Total stat in the UI.
        
        Parameters:
            total (int): The total count of items to be displayed.
        """
    ...
    def _process_log_queue(self) -> None:
        """
        Drain the internal log queue and forward queued messages to the preprocessing LogViewer.
        
        Each queued item is a (level, message) tuple; messages are dispatched to the LogViewer using the corresponding level method: 'info', 'warning', 'error', or 'success'.
        """
        log = self.query_one("#preprocess-log", LogViewer)
    ...
    def _on_complete(self, success: bool) -> None:
        """
        Finalize the preprocessing UI after the run completes.
        
        Disables the Cancel button, enables the Back button, and shows a notification indicating success or that preprocessing was stopped/failed.
        
        Parameters:
            success (bool): `True` if preprocessing completed successfully, `False` if it was cancelled or failed.
        """
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        """
        Dispatch actions for header buttons based on the pressed button's id.
        
        Parameters:
            event (Button.Pressed): The button press event; invokes `action_cancel` when event.button.id == "btn-cancel" and `action_back` when event.button.id == "btn-back".
        """
    ...
    def action_cancel(self) -> None:
        """
        Mark the preprocessing operation as cancelling and initiate cancellation.
        
        Sets an internal cancelling flag, enqueues a warning in the preprocess log, shows a short user notification, and requests cancellation of the background worker if one is active.
        """
        if self._is_cancelling:
            return
