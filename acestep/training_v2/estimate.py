...
def _find_attention_modules(model: nn.Module, granularity: str) -> List[str]:
    """
    Identify attention-related module names in a model for targeting (e.g., LoRA or sensitivity analysis).
    
    Parameters:
        model (nn.Module): The model to inspect.
        granularity (str): Selection mode: "module" to collect individual attention projection submodules whose names contain "to_q", "to_k", "to_v", or "to_out"; any other value selects attention blocks at the layer level (modules with "attn" in the name, excluding projection and normalization submodules).
    
    Returns:
        List[str]: A list of module names that match the requested attention-related patterns.
    """
    modules = []
