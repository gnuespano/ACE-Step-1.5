...
class RichHelpFormatter(argparse.HelpFormatter):
    ...
    def __init__(
        ...
        """
        Initialize the Rich-enabled help formatter and resolve the formatter width.
        
        Parameters:
            prog: The program name used by the base formatter.
            indent_increment: Number of spaces to increase indentation for each level.
            max_help_position: Column at which help text starts for each action.
            width: Target output width in characters. If `None`, attempts to use Rich's Console width; if Rich is unavailable, defaults to 80.
        """
        if width is None:
            try:
    ...
    def format_help(self) -> str:
        """
        Format the parser's help text using Rich styles when Rich is available.
        
        Attempts to render the help produced by the base HelpFormatter with Rich styling for sections, arguments, subcommands, and the usage line; if the Rich library is not installed, returns the original unstyled help text.
        
        Returns:
            str: The formatted help text (Rich-styled when possible, otherwise the raw help string).
        """
