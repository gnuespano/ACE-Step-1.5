...
def run_wizard() -> Optional[argparse.Namespace]:
    """
    Present an interactive top-level wizard and dispatch to the selected flow.
    
    Displays the interactive banner, prompts the user to choose an action (train fixed/vanilla, preprocess, estimate, or exit), and invokes the corresponding interactive flow.
    
    Returns:
        An `argparse.Namespace` populated by the chosen flow, or `None` if the user chose Exit or aborted the wizard.
    """
