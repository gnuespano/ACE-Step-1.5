...
def wizard_train(mode: str) -> argparse.Namespace:
    """
    Launches an interactive wizard to collect training configuration.
    
    Prompts the user for required, LoRA, training, logging, and (optionally) advanced settings,
    then normalizes and validates inputs and assembles them into a Namespace that mirrors the
    CLI's training arguments. The returned Namespace has its `subcommand` set to the provided
    mode and includes flags `_from_wizard=True`, `plain=False`, and `yes=True`. Advanced
    options (device, precision, optimizer, scheduler, data-loading, logging, VRAM savings,
    etc.) are exposed when the user chooses the advanced configuration mode.
    
    Parameters:
        mode (str): Which training flow to prepare; typically "fixed" or "vanilla" (affects which
            corrected-training prompts are shown).
    
    Returns:
        argparse.Namespace: A populated Namespace containing all collected training configuration
        values (paths, model and LoRA settings, training hyperparameters, data-loading and
        logging options, and various defaults) suitable for passing to the training CLI.
    """
...
def wizard_preprocess() -> argparse.Namespace:
    """
    Builds an argparse.Namespace for the preprocessing subcommand by interactively prompting the user for preprocessing settings.
    
    The wizard prompts for checkpoint directory, model variant, source audio directory, an optional dataset JSON path (treated as None if left empty or set to "None"; a warning is shown if a provided path does not exist), output directory for generated tensor (.pt) files, and max audio duration. The returned Namespace mirrors the CLI preprocessing configuration and includes sensible defaults for training and data-loading related fields.
    
    Returns:
        argparse.Namespace: Namespace containing preprocessing configuration with subcommand="fixed", preprocess=True, dataset_dir set to the chosen tensor output directory, and populated fields such as checkpoint_dir, model_variant, audio_dir, dataset_json (or `None`), tensor_output, max_duration, num_workers, pin_memory, learning_rate, batch_size, rank, alpha, dropout, target_modules, attention_type, output_dir, and other logging/checkpointing and optimizer defaults.
    """
...
def wizard_estimate() -> argparse.Namespace:
    """
    Start an interactive wizard to collect options for gradient sensitivity estimation.
    
    Prompts the user for checkpoint directory, model variant, dataset directory, estimation
    parameters (number of batches, top-K, granularity), LoRA settings (rank, alpha, dropout),
    and an output JSON path, then assembles these and additional defaults into an argparse.Namespace
    suitable for running the `estimate` subcommand.
    
    Returns:
        argparse.Namespace: Namespace configured for the "estimate" subcommand containing the
        collected estimation parameters (`estimate_batches`, `top_k`, `granularity`), LoRA settings
        (`rank`, `alpha`, `dropout`), `estimate_output` path, and other CLI-equivalent default fields.
    """
