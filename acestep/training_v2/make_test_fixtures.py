...
def generate_single_sample(
    ...
    """
    Generate a single synthetic preprocessed sample with tensors shaped for the DiT decoder.
    
    Parameters:
        sample_id (str): Identifier placed into the sample metadata and used for filenames.
        latent_length (int): Sequence length T for `target_latents` and `context_latents`.
        encoder_length (int): Sequence length L for `encoder_hidden_states`.
        dtype (torch.dtype): Data type for all generated tensors.
    
    Returns:
        dict: A mapping with keys:
            - `target_latents` (Tensor[T, 64]): Random latents for the decoder target.
            - `attention_mask` (Tensor[T]): Ones mask for `target_latents`.
            - `encoder_hidden_states` (Tensor[L, 2048]): Random encoder hidden states.
            - `encoder_attention_mask` (Tensor[L]): Ones mask for `encoder_hidden_states`.
            - `context_latents` (Tensor[T, 128]): Random context latents concatenated components.
            - `metadata` (dict): Contains `sample_id`, `caption`, `lyrics`, `duration` (seconds), and `is_synthetic` flag.
    """
    return {
...
def generate_fixtures(
    ...
    """
    Create `num_samples` synthetic .pt fixture files and a manifest.json in `output_dir`.
    
    Parameters:
        output_dir (Path): Directory to write fixture files and manifest to; created if missing.
        num_samples (int): Number of synthetic samples to generate.
        latent_length (int): Latent sequence length (T dimension) for each sample.
        encoder_length (int): Encoder sequence length (L dimension) for each sample.
    
    Returns:
        Path: Path to the directory containing the generated fixtures.
    """
...
def main() -> int:
    """
    CLI entry point that parses command-line arguments and generates synthetic preprocessed tensor fixtures.
    
    Accepts --output-dir, --num-samples, --latent-length, and --encoder-length to control where fixtures are written and their sizes; invokes the fixture generation process with the parsed values.
